...
drawWorld world
  ...
  | otherwise   = draw world
    where draw world = drawSnake (snake world)
          ...
          score_str = "Player1 Score: " ++ (show $ score $ snake world)

handleEvent :: Event -> World -> World
handleEvent (TimePassing dt) w
  | state w == GameOver = w
  | otherwise = handleSnakeAction snake' action w
    where (snake', action) = checkSnakeAction dt w
...
randomApple world@(World rnds _ _ apple w h) = (mkApple pos color, rnds')
