...
instance Show BinaryPolynom where
    show (BPoly []) = "0"
    show (BPoly ps) = intercalate " + " . map showTerm . filter ((/=0) . fst)
...
addition (BPoly ps1) (BPoly ps2) = BPoly $ psAddLE ps1 ps2
...
x .+. y = snd $ polyDivMod (BPoly $ psAddLE (unPoly x) (unPoly y)) irrePolyF2e8
